    ...
    for index, train_this_iter_config in enumerate(train_config["rl_train"]):
        ...
        if index > 0:
            ...
            if not THIS_ITER_RESET_REPLAY_BUFFER:
                ...
                substitute_sac_algo.load_replay_buffer(policy_save_dir / train_config["rl_train"][index-1]["rl"]["experiment_name"] / replay_buffer_save_name)
                #sac_algo.load_replay_buffer(policy_save_dir / train_config["rl_train"][index-1]["rl"]["experiment_name"] / replay_buffer_save_name)
                print(f"Iter {index}: load substitute replay buffer from {policy_save_dir / train_config['rl_train'][index-1]['rl']['experiment_name'] / replay_buffer_save_name}.")
                ...
                if THIS_ITER_RELABEL_REPLAY_BUFFER:
                    ...
                    if not THIS_ITER_WRAPPER_LIST :
                        ...
                        if contains_frame_skip:
                            loaded_replay_buffer_size = substitute_sac_algo.replay_buffer.size()
                            
                            new_rewards = []

                            for info in substitute_sac_algo.replay_buffer.infos:
                            ...
                            substitute_sac_algo.replay_buffer.rewards[:loaded_replay_buffer_size] = new_rewards.reshape(-1, 1)
                            substitute_sac_algo.replay_buffer.infos = np.array([[{} for _ in range(sac_algo.replay_buffer.n_envs)] for _ in range(sac_algo.replay_buffer.buffer_size)])
                            substitute_sac_algo.save_replay_buffer(str(PROJECT_ROOT_DIR / "checkpoints" / THIS_ITER_RL_EXPERIMENT_NAME / "new_replay_buffer"))
                            sac_algo.load_replay_buffer(str(PROJECT_ROOT_DIR / "checkpoints" / THIS_ITER_RL_EXPERIMENT_NAME / "new_replay_buffer"))

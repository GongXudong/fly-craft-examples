...
def weight_deviation(dic1, dic2):
    loss = 0
    mse_loss = nn.MSELoss()
    with torch.no_grad():
        for (n1, p1), (n2, p2) in zip(dic1.items(), dic2.items()):
            if n1 != n2:
                raise ValueError("Parameter names are different.")
            loss += mse_loss(p1.data, p2.data)
    return loss

def weight_hessian(loss, param):
    grad, = autograd.grad(loss, param, retain_graph = True, create_graph = True)
    grad = grad.reshape(-1)
    lst = []
    for g in grad:
        h, = autograd.grad(g, param, retain_graph = True)
        lst.append(h.reshape(-1))
    return torch.stack(lst)

def feature_rank(tensor, eps):
    sv = torch.linalg.svdvals(tensor)
    return torch.sum(sv > eps).item()

...
from train_scripts.ladderrl.trys.dormant_callback import  DormantNeuronMonitorCallback
import warnings
...
def train(train_config):
    ...
    for index, train_this_iter_config in enumerate(train_config["rl_train"]):
        ...
        THIS_ITER_RESET_SCOPE = train_this_iter_config["rl"].get("reset_scope", "none")
        ...
        if (index > 0 and THIS_ITER_RESET_POLICY):
            ...
            if THIS_ITER_RESET_SCOPE == "all":
                ...
                if THIS_ITER_RESET_SCOPE == "actor":
                    ...
                    # 1. 替换 Actor 网络 (权重)
                    sac_algo.policy.actor = dummy_algo.policy.actor
                    # 2. 替换 Actor 优化器 
                    sac_algo.actor = sac_algo.policy.actor

                    sac_algo.policy.actor.optimizer = dummy_algo.policy.actor.optimizer
                
                elif THIS_ITER_RESET_SCOPE == "critic":
                    print(f"Iter {index}: >>> Resetting CRITIC network and optimizer <<<")
                    # 1. 替换 Critic 网络
                    sac_algo.policy.critic = dummy_algo.policy.critic
                    # 2. 替换 Critic Target 网络 (SAC 有 Target Net，必须一起重置)
                    sac_algo.policy.critic_target = dummy_algo.policy.critic_target
                    # 3. 替换 Critic 优化器
                    sac_algo.critic = sac_algo.policy.critic
                    sac_algo.critic_target = sac_algo.policy.critic_target
                    sac_algo.policy.critic.optimizer = dummy_algo.policy.critic.optimizer
        ...
        # dormant callback
        dorm_callback = DormantNeuronMonitorCallback()

...
def rollout(
    ...
    for index, target in tqdm(target_goals.iterrows(), total=target_goals.shape[0]):
        ...
        res_dict["error"].append(angle_of_2_velocity(v_1=target["v"], mu_1=target["mu"], chi_1=target["chi"], v_2=info["plane_next_state"]["v"], mu_2=info["plane_next_state"]["mu"], chi_2=info["plane_next_state"]["chi"]))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--eval-traj-num", type=int, help="测试使用的轨迹数", default=10000)
    parser.add_argument("--process-num", type=int, help="使用的采样进程数", default=16)
    parser.add_argument("--seed", type=int, help="随机种子", default=42)
    args = parser.parse_args()

    # parser.add_argument("--v_min",type=float,help="v_min",default=100.0)
    # parser.add_argument("--v_max",type=float,help="v_max",default=300.0)

    # parser.add_argument("--mu_min",type=float,help="mu_min",default=-85.0)
    # parser.add_argument("--mu_max",type=float,help="mu_max",default=85.0)

    # parser.add_argument("--chi_min",type=float,help="chi_min",default=-170.0)
    # parser.add_argument("--chi_max",type=float,help="chi_max",default=170.0)
    # parser.add_argument("--save-file-name", type=str, help="", default="res.csv")
#     evaluation_goals = pd.DataFrame({
#     "v": [np.random.random() * (args.v_max - args.v_min) + args.v_min for i in range(args.eval_traj_num)],
#     "mu": [np.random.random() * (args.mu_max - (args.mu_min)) + (args.mu_min) for i in range(args.eval_traj_num)],
#     "chi": [np.random.random() * (args.chi_max - args.chi_min) + args.chi_min for i in range(args.eval_traj_num)],
# })
    ...
    # "chi_max": 170.0,
    evaluation_goals = pd.DataFrame({
        "v": [np.random.random() * (300. - 100.) + 100. for i in range(args.eval_traj_num)],
        "mu": [np.random.random() * (85. - (-85.)) + (-85.) for i in range(args.eval_traj_num)],
        "chi": [np.random.random() * (170. - (-170.)) + (-170.) for i in range(args.eval_traj_num)],
    })
    ...
    #     "mu": [np.random.random() * (30. - (-30.)) + (-30.) for i in range(args.eval_traj_num)],
    #     "chi": [np.random.random() * (60. - (-60.)) + (-60.) for i in range(args.eval_traj_num)],
    # })
    ...
    for config_file_name, algo_str, save_file_name in zip(
        ...
        with Pool(processes=args.process_num) as pool:
            ...
            env_config = load_config(PROJECT_ROOT_DIR / "configs" / "env" / train_config["rl_train"][-1]["env"].get("config_file", "env_config_for_sac.json"))
            ...
            res = pool.starmap(
                rollout,
                [[
                    str(PROJECT_ROOT_DIR / "checkpoints" /  train_config["rl_train"][-1]["rl"]["experiment_name"]),
                    algo_str,
                    str(PROJECT_ROOT_DIR / "configs" / "env" / train_config["rl_train"][-1]["env"]["config_file"]),
                    list(target.v),
                    list(target.mu),
                    list(target.chi),
                    False,

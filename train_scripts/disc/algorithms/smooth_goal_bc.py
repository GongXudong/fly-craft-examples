...
class BehaviorCloningLossCalculator:
    ...
    def init_desired_goal_params(self, helper_env: gym.Env=None, goal_noise_epsilon: np.array = np.array([1.0, 0.3, 0.3]), device: Union[str, th.device] = "cuda"):
        ...
        self.desired_goal_max = th.tensor(self.desired_goal_max, requires_grad=False, device=device)
        ...
        self.desired_goal_min = th.tensor(self.desired_goal_min, requires_grad=False, device=device)

        self.noise_max = helper_env.env.goal_scalar.transform(goal_noise_epsilon.reshape((1, -1))).reshape((-1)) - np.array([0., 0.5, 0.5])
        self.noise_min = - self.noise_max.copy()
        self.noise_max = th.tensor(self.noise_max, requires_grad=False, device=device)
        self.noise_min = th.tensor(self.noise_min, requires_grad=False, device=device)
    
    def add_noise_to_desired_goals(self, observations, device: Union[str, th.device] = "auto") -> None:
        observations["desired_goal"] = th.clamp(
            input=observations["desired_goal"] + th.rand(size=observations["desired_goal"].shape, requires_grad=False, device=device) * (self.noise_max - self.noise_min) + self.noise_min,
    ...
    def __call__(
        ...
        features = policy.extract_features(tensor_obs)
        ...
        # 2.get action dist
        noised_goal_action_dist = policy.get_distribution(noised_goal_obs)

        # 3.calc KL or JS loss
        if self.policy_distance_measure_func == "KL":
            noised_goal_discrepency = th.distributions.kl_divergence(action_dist, noised_goal_action_dist.distribution).sum(axis=-1).mean()
        elif self.policy_distance_measure_func == "JS":
            noised_goal_discrepency = th.distributions.kl_divergence(action_dist, noised_goal_action_dist.distribution).sum(axis=-1) + th.distributions.kl_divergence(noised_goal_action_dist.distribution, action_dist).sum(axis=-1)
            noised_goal_discrepency = noised_goal_discrepency.mean()
        else:
        ...
        noise_goal_loss = self.ng_weight * noised_goal_discrepency
        loss = neglogp + ent_loss + l2_loss + noise_goal_loss
        ...
        return BCTrainingMetrics(
            ...
            l2_norm=l2_norm,
            l2_loss=l2_loss,
            ng_discrepency=noised_goal_discrepency,
            ng_loss=noise_goal_loss,
            loss=loss,
        )
...
class SmoothGoalBC(algo_base.DemonstrationAlgorithm):

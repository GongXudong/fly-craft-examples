...
def calc_KL(policy: SACMultiInputPolicy, new_desired_goal: th.Tensor, obs_list: List[th.Tensor], action_dist_list: List[Distribution]):
    KL_list = []
    for obs, reference_action_distribution in zip(obs_list, action_dist_list):
        ...
        actions = policy._predict(obs_tensor, deterministic=True)
...
def evaluate(args):
    ...
    for tmp_seed in tqdm(args.algo_seeds, total=len(args.algo_seeds)):
        ...
        if args.algo_class == "SAC":
            policy_class = SAC
        elif args.algo_class == "SmoothGoalSAC":
            policy_class = SmoothGoalSAC
        else:
            raise ValueError("algo_class can only be SAC or SmoothGoalSAC!")

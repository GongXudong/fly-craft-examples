...
class SmoothGoalPPOMaxRandom(PPO):
    ...
    def train(self) -> None:
        ...
        for epoch in range(self.n_epochs):
            ...
            for rollout_data in self.rollout_buffer.get(self.batch_size):
                ...
                # 对比smooth_goal_ppo.py, 只多了下面这一行代码！！！
                # 每个(state, goal)对应noise_num_for_each_goal个噪声，只用KL最大的计算loss
                # 执行下面的语句前，noised_goal_kl的shape为：(self.batch_size * self.noise_num_for_each_goal, )，执行之后其shape为：(self.batch_size,)
                noised_goal_kl, _ = noised_goal_kl.reshape((self.batch_size, self.noise_num_for_each_goal)).max(dim=1)

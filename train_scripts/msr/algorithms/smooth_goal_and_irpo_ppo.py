...
class SmoothGoalAndIRPOPPO(PPO):
    ...
    def __init__(
        ...
        regularize_kl_with_pretrained_policy: bool = False,
        ...
        self.regularize_kl_with_pretrained_policy = regularize_kl_with_pretrained_policy
    ...
    def train(self) -> None:
        ...
        for epoch in range(self.n_epochs):
            ...
            for rollout_data in self.rollout_buffer.get(self.batch_size):
                ...
                # print(distribution.distribution.loc.shape, distribution.distribution.scale.shape)
                ...
                original_action_dist = distribution.distribution

                repeated_action_dist = th.distributions.Normal(
                    loc=original_action_dist.loc.repeat((self.noise_num_for_each_goal, 1)),
                    scale=original_action_dist.scale.repeat((self.noise_num_for_each_goal, 1))
                )
                ...
                if self.policy_distance_measure_func == "KL":
                    noised_goal_kl = th.distributions.kl_divergence(repeated_action_dist, noised_goal_action_dist.distribution).sum(axis=-1)
                elif self.policy_distance_measure_func == "JS":
                    noised_goal_kl = th.distributions.kl_divergence(repeated_action_dist, noised_goal_action_dist.distribution).sum(axis=-1) + th.distributions.kl_divergence(noised_goal_action_dist.distribution, repeated_action_dist).sum(axis=-1)
                else:
                ...
                # 3.calculate kl loss with the bc-trained model
                ...
                # )
                kl_loss_with_bc_model = th.distributions.kl_divergence(original_action_dist, bc_trained_algo_distribution.distribution).sum(axis=-1)
                ...
                if self.regularize_kl_with_pretrained_policy:
                    loss += self.kl_coef_with_bc * kl_loss

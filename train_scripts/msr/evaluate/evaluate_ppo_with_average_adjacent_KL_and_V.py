...
def evaluate(args):
    ...
    for tmp_seed in tqdm(args.algo_seeds, total=len(args.algo_seeds)):
        ...
        for _ in tqdm(range(args.evaluate_dg_num), total=args.evaluate_dg_num):
            # 找到一个能完成的goal
            achieved, achievable_dg, discounted_cumulative_reward, obs_list, action_dist_list = attacker.get_an_achievable_desired_goal(
            ...
            for tmp_mu in np.linspace(noise_min[1], noise_max[1], 2*args.evaluate_adjacent_num+1):
                ...
                for tmp_chi in np.linspace(noise_min[2], noise_max[2], 2*args.evaluate_adjacent_num+1):
                    ...
                    tmp_success, _, noised_goal_discounted_cumulative_reward = my_evaluate_with_customized_dg(
                        policy=algo.policy, 
                        env=env, 
...
if __name__ == "__main__":
    ...
    parser.add_argument("--evaluation-gamma", type=float, default=0.99, help="gamma used in calculating the discounted cumulative rewards in evaluation")
    # save res file

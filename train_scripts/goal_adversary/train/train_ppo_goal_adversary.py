...
from train_scripts.goal_adversary.utils.vec_env_helper import get_goal_adversary_efficient_vec_env
...
def train():
    ...
    if GOAL_ADVERSARY_REFERENCE_ALGO_TYPE == "ppo":
        referenced_algo_class = PPO
    else:
        raise ValueError("Unsupported GOAL_ADVERSARY_REFERENCE_ALGO_TYPE: {}".format(GOAL_ADVERSARY_REFERENCE_ALGO_TYPE))
    ...
    print(f"type: {GOAL_ADVERSARY_REFERENCE_ALGO_TYPE}, referenced algo path: {GOAL_ADVERSARY_REFERENCE_ALGO_PATH}")
    ...
    env_config_dict_in_training = {
        ...
        "algo_class": referenced_algo_class,
        "algo_path": str(PROJECT_ROOT_DIR / GOAL_ADVERSARY_REFERENCE_ALGO_PATH),
    ...
    vec_env = VecCheckNan(get_goal_adversary_efficient_vec_env(
        **env_config_dict_in_training
    ))
    # evaluate_policy使用的测试环境
    eval_env = VecCheckNan(get_goal_adversary_efficient_vec_env(
        **env_config_dict_in_eval
    ))
    # 回调函数中使用的测试环境
    eval_env_in_callback = VecCheckNan(get_goal_adversary_efficient_vec_env(
    ...
    sb3_logger.log(f"Check goal adversary configs, algorithm path: {GOAL_ADVERSARY_REFERENCE_ALGO_PATH}, goal noise min: {GOAL_ADVERSARY_NOISE_MIN}, goal noise max: {GOAL_ADVERSARY_NOISE_MAX}.")
...
if __name__ == "__main__":
    ...
    GOAL_ADVERSARY_REFERENCE_ALGO_TYPE = train_config["env"].get("referenced_algo_type", "ppo")
    GOAL_ADVERSARY_REFERENCE_ALGO_PATH = train_config["env"].get("referenced_algo_path", "")

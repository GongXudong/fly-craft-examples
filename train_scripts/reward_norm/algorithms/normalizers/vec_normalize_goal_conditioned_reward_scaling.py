...
class VecNormalizeGoalConditionedRewardScaling(VecEnvWrapper):
    """
    ...
    def __init__(
        ...
        self.ret_rms = {}  # key: goal_str, value: RunningMeanStd(shape=())
    ...
    def get_goal_repr(self, desired_goal: np.ndarray) -> str:
        assert len(desired_goal.shape) == 1, "desired_goals must by 1D array."
        
        return '_'.join(desired_goal.astype(int).astype(str).tolist())

    def get_cluster_center_of_desired_goal(self, desired_goal: np.ndarray) -> np.ndarray:
        return np.round(desired_goal)
    ...
    def step_wait(self) -> VecEnvStepReturn:
        ...
        assert "desired_goal" in obs, "obs must contain key: desired_goal"
        ...
        if self.training:
            self._update_reward(rewards, obs["desired_goal"])
        rewards = self.normalize_reward(rewards, obs["desired_goal"])
    ...
    def _update_reward(self, reward: np.ndarray, desired_goals: np.ndarray) -> None:
        """Update reward normalization statistics."""
        self.returns = self.returns * self.gamma + reward
        
        for r, dg in zip(reward, desired_goals):
            cluster_center = self.get_goal_repr(self.get_cluster_center_of_desired_goal(desired_goal=dg))
            
            if cluster_center not in self.ret_rms:
                self.ret_rms[cluster_center] = RunningMeanStd(shape=())

            self.ret_rms[cluster_center].update(np.array([r]))
    ...
    def normalize_reward(self, reward: np.ndarray, desired_goals: np.ndarray) -> np.ndarray:
        """
        ...
        if self.norm_reward:
            ...
            for r, dg in zip(reward, desired_goals):
                cluster_center = self.get_goal_repr(self.get_cluster_center_of_desired_goal(desired_goal=dg))
                if cluster_center in self.ret_rms:
                    tmp_reward = np.clip(r / np.sqrt(self.ret_rms[cluster_center].var + self.epsilon), -self.clip_reward, self.clip_reward)
                else:
                    print(f"Warning in normalize_reward func: {cluster_center} is not in the dict of Reward Scaling Dict!!!")
                    tmp_reward = r
                
                normarlized_rewards.append(tmp_reward)
            
            return np.array(normarlized_rewards)
    ...
    def unnormalize_reward(self, reward: np.ndarray, desired_goals: np.ndarray) -> np.ndarray:
        if self.norm_reward:
            unnormarlized_rewards = []
            for r, dg in zip(reward, desired_goals):
                cluster_center = self.get_goal_repr(self.get_cluster_center_of_desired_goal(desired_goal=dg))
                if cluster_center in self.ret_rms:
                    tmp_reward = r * np.sqrt(self.ret_rms[cluster_center].var + self.epsilon)
                else:
                    print(f"Warning in unnormalize_reward func: {cluster_center} is not in the dict of Reward Scaling Dict!!!")
                    tmp_reward = r
                unnormarlized_rewards.append(tmp_reward)
            
            return np.array(unnormarlized_rewards)
    ...
    @staticmethod
    def load(load_path: str, venv: VecEnv) -> "VecNormalizeGoalConditionedRewardScaling":
        """

...
class MyPandaPickAndPlaceEnv(RobotTaskEnv):
    ...
    def __init__(
        ...
        goal_xy_range: float = 0.5,
        goal_z_range: float = 0.2,
        obj_xy_range: float = 0.3,
    ) -> None:
        sim = PyBullet(render_mode=render_mode, renderer=renderer)
        robot = Panda(sim, block_gripper=False, base_position=np.array([-0.6, 0.0, 0.0]), control_type=control_type)
        task = PickAndPlace(sim, reward_type=reward_type, distance_threshold=distance_threshold, goal_xy_range=goal_xy_range, goal_z_range=goal_z_range, obj_xy_range=obj_xy_range)
        super().__init__(
            robot,
            task,

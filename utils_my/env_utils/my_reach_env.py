...
class MyPandaReachEnv_20(RobotTaskEnv):
    """Reach task wih Panda robot.
...
class MyPandaReachEnv_10(RobotTaskEnv):
    """Reach task wih Panda robot.
...
class MyPandaReachEnv_5(RobotTaskEnv):
    """Reach task wih Panda robot.
...
class MyPandaReachEnv_3(RobotTaskEnv):
    """Reach task wih Panda robot.
    ...
    def __init__(
        ...
        sim = PyBullet(render_mode=render_mode,n_substeps = 3, renderer=renderer)
        robot = Panda(sim, block_gripper=True, base_position=np.array([-0.6, 0.0, 0.0]), control_type=control_type)
        task = ChangeGoalReach(sim, reward_type=reward_type, get_ee_position=robot.get_ee_position, distance_threshold=distance_threshold, goal_range=goal_range)
        super().__init__(
            robot,
...
class MyPandaReachEnv_1(RobotTaskEnv):
    """Reach task wih Panda robot.
    ...
    def __init__(
        ...
        sim = PyBullet(render_mode=render_mode,n_substeps = 1, renderer=renderer)
        robot = Panda(sim, block_gripper=True, base_position=np.array([-0.6, 0.0, 0.0]), control_type=control_type)
        task = ChangeGoalReach(sim, reward_type=reward_type, get_ee_position=robot.get_ee_position, distance_threshold=distance_threshold, goal_range=goal_range)
        super().__init__(
            robot,
